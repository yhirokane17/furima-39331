  ...
  describe '出品の投稿' do
    ...
    context '出品投稿ができない場合' do
      ...
      it '商品の状態が---では登録できない' do
        @item.status_id = '1'
        @item.valid?
        expect(@item.errors.full_messages).to include "Status id is invalid"
      end
      it '配送料の負担が空では登録できない' do
        @item.delivery_price_id = ''
        @item.valid?
        expect(@item.errors.full_messages).to include "Delivery price id can't be blank"
      end
      it '配送料の負担が---では登録できない' do
        @item.delivery_price_id = '1'
        @item.valid?
        expect(@item.errors.full_messages).to include "Delivery price id is invalid"
      end
      it '配送元の地域が空では登録できない' do
        @item.address_prefecture_id = ''
        @item.valid?
        expect(@item.errors.full_messages).to include "Address prefecture id can't be blank"
      end
      it '配送料の負担が---では登録できない' do
        @item.address_prefecture_id = '1'
        @item.valid?
        expect(@item.errors.full_messages).to include "Address prefecture id is invalid"
      end
      it '配送までの日数が空では登録できない' do
        @item.delivery_date_id = ''
        @item.valid?
        expect(@item.errors.full_messages).to include "Delivery date id can't be blank"
      end
      it '配送までの日数が---では登録できない' do
        @item.delivery_date_id = '1'
        @item.valid?
        expect(@item.errors.full_messages).to include "Delivery date id is invalid"
      end
      it '価格が空では登録できない' do
        @item.price = ''
        @item.valid?
